...
**metapip** installs, inspects, and manages the R packages behind the World
Bank's [Poverty and Inequality Platform (PIP)](https://pip.worldbank.org/).
Think of it as the [{tidyverse}](https://github.com/tidyverse/tidyverse) for
PIP: one package that loads and coordinates all the others.
...
`init_metapip()` checks each package against its configured branch and
installs any outdated packages. When a `PIP_LOCK.csv` lockfile is present,
it installs from the exact recorded SHAs.
