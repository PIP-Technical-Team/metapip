...
|----|-------------|--------|
| R1 | `get_branches()` returns all branches even for repos with >30 branches (`per_page = 100`, `.limit = 100`) | S4 |
| R2 | Codecov badge URL in README carries no `?token=` parameter; `README.Rmd` and `README.md` stay in sync | S3 |
| R3 | New `gh_token()` helper returns the GitHub token when available and `NULL` when missing, with no error/abort | S2 |
...
| R7 | `install_branch()` pins to the resolved branch HEAD SHA by default and installs via `@<sha>` | S1-A |
...
| R10 | New `pip_snapshot()` resolves all core package branches to current HEAD SHAs and writes/updates `PIP_LOCK.csv` (`package,branch,sha` with header) | S1-B |
...
| R17 | `NEWS.md` updated; `man/`+`NAMESPACE` regenerated for the new export and changed signatures | Conventions |
| R18 | All changes ship as a single mergeable PR targeting `master` | Requirement (PR) |
...
- **Details**:
  - In `get_branches()` (`R/get_branches.R:22-24`), add `per_page = 100` and
    `.limit = 100` to the `gh::gh("GET /repos/PIP-Technical-Team/{repo}/branches", ...)`
    call. This covers repos with up to 100 branches (well beyond any realistic
    PIP repo) and prevents silent branch truncation at 30.
  - Keep the existing `.token = creds$password` for now (Step 3 / S2 will
    replace it with the `gh_token()` best-effort helper). This step changes
    only the pagination arguments so it stays minimal and conflict-free with
    the later token refactor.
...
- **Tests**: `tests/testthat/test-get_branches.R` — extend the existing
  ...
  received `per_page = 100` and `.limit = 100` via `mockery::mock()` capture if
  feasible; otherwise assert the non-truncated count.
...
### 3. Least-privilege tokens — gh_token()/gh_auth() + redacted check_github_token() (S2)
...
- **Details**:
  - New `gh_token()` helper in `R/utils.R` (internal, `@keywords internal`):
    safely attempt `gitcreds::gitcreds_get()` inside `tryCatch`; on success
    return `creds$password` (the token string); on `gitcreds_nogit_error` or
    `gitcreds_no_credentials` (or any error), return `NULL`. Never abort. Also
    honor `Sys.getenv("GITHUB_PAT")` / `"GITHUB_TOKEN"` as a fallback before
    gitcreds, returning the first non-empty value.
  - New `gh_auth()` helper (internal, `@keywords internal`): a thin wrapper
    that decides token presence for `gh::gh()` call sites. It returns the value
    of `gh_token()` (token string or `NULL`) — `NULL` makes `gh::gh()` operate
    unauthenticated, which is valid for the public `PIP-Technical-Team` org.
    This centralizes the "use token if present, else unauthenticated" decision.
  - `check_github_token()` (`R/utils.R:91-106`): keep it as the STRICT gate for
    install functions — it still aborts when no credentials are available (an
    install genuinely needs auth for higher rate limits / private fallbacks).
    Change its return value to a REDACTED list: return
    `list(name = "", password = "", protocol = "", ...)` with `password = ""`
    (and any secret fields blanked), and add a `print` method or coerce so
    `print(check_github_token())` shows `""` instead of the PAT. Recommended:
    give the return a class (e.g. `c("metapip_token", "list")`) and add a
    `print.metapip_token` method that prints redacted fields. The REAL token
    must still be obtainable internally where an install needs it — but
    `remotes::install_github()` resolves gitcreds itself, so `check_github_token()`
    remains a validation gate, not a token carrier. Document the redaction in
    the `@return` roxygen.
  - Remove `.token = creds$password` from read-only `gh::gh()` calls and
    replace with `.token = gh_token()` (best-effort: passes the token if
    present, `NULL` if absent). Call sites: `get_branches()`
    (`R/get_branches.R:22-24`); the `core_metadata()` releases/commits calls in
    `R/core_metadata.R`; `package_branches()`'s underlying `get_branches()`
    call is covered transitively. Remove the leading `check_github_token()`
    call from `get_branches()`, `get_branch_info()`,
    `get_latest_branch_update()`, `core_metadata()`, and `package_branches()`
    — these are read-only for a public org and must work without a PAT.
...
- **Details**:
  ...
  - Behavior when `force = FALSE` (default):
    1. Resolve the target SHA: `target_sha <- sha %||% latest_commit_for_branch(package, branch)$sha`.
...
- **Test Scenarios**:
  ...
  - edge case (idempotent): installed `RemoteSha` == resolved SHA →
  ...
  - edge case (no local install): `packageDescription` returns `NA` →
...
- **Tests**: `tests/testthat/test-get_branches.R` — rewrite/extend the
  existing "install_branch works correctly" `test_that`. Stub
  `latest_commit_for_branch` to return `list(sha = "abc123")`, stub
  `remotes::install_github` (capture args), stub `check_github_token`,
  `get_branches`, `detach_package`. Assert: default call installs with an
  arg containing `@abc123`; `force=TRUE` installs with an arg containing
  `@<branch>` (not `@abc123`) and a warning is emitted; idempotent case —
  stub `utils::packageDescription` to return `"abc123"` and assert
  `remotes::install_github` is NOT called and an info "skipping" is emitted.
- **Acceptance criteria**: V7, V8 pass; default install is SHA-pinned and
  idempotent; `force=TRUE` installs live HEAD.
...
- **Details**:
  - New exported `pip_snapshot(path = NULL)`:
    - Default `path` via an internal `pip_lock_path()` helper that returns
      `system.file("PIP_LOCK.csv", package = "metapip")` (the `inst/` location,
      which under `load_all` points to the source `inst/` so maintainers can
      commit updates; under an installed package it points to the library
      copy). `pip_lock_path()` is `@keywords internal`.
...
- **Details**:
  - `init_metapip()`: change from a pure delegate to lock-driven:
    1. `lock_path <- pip_lock_path()`. If `file.exists(lock_path)`: read
       `lock <- utils::read.csv(lock_path, stringsAsFactors = FALSE)`. For each
       row, call `install_branch(package = lock$package[i],
...
- **Details**:
  ...
  - In the per-package install decision (after the existing `compare_sha`
    ...
    branch HEAD (consistent with its "latest branch" intent). The `force=TRUE`
    warning from Step 4 may be suppressed here to avoid double-warning, OR
    kept — choose one and test accordingly (recommended: suppress the inner
    `force` warning here since `install_latest_branch` already warned about
    bypassing the lock).
...
- **Files**: `tests/testthat/test-init_metapip.R`, `NEWS.md`, `man/`, `NAMESPACE`, `DESCRIPTION` (only if a new dependency is needed — none expected)
- **Details**:
...
- [ ] `R/utils.R` — roxygen for new `gh_token()`, `gh_auth()` (`@keywords
      internal`), and updated `check_github_token()` `@return` (redaction).
...
- [ ] `man/` + `NAMESPACE` — regenerated via `devtools::document()`;
      `pip_snapshot` exported, `gh_token`/`gh_auth` internal.

## Risks & Mitigations

| ID | Risk | Impact | Mitigation |
|----|------|--------|------------|
| K1 | S2 (Step 3) and S4 (Step 1) both edit `get_branches()`'s `gh::gh` call | merge conflict / rework | C2 ordering: Step 1 adds pagination only (keeps `.token`), Step 3 rewrites the token. Keep commits ordered S4 → S2. |
...
| K7 | `system.file("PIP_LOCK.csv")` under `load_all` points to source `inst/` — `pip_snapshot()` default writes there, which may be read-only in some setups | write failure on installed package | Provide `path` arg override; tests use `tempfile()`; document the maintainer workflow (run `pip_snapshot()` in a `load_all` dev session, then commit `inst/PIP_LOCK.csv`). |
...
|----|-------------------|------------------|-------|----------|
| V1 | `get_branches("pipapi")` returns all branches even if >30 exist (`per_page=100`, `.limit=100`) | `test-get_branches.R` (mock gh returning 45 branches) | 1 | yes |
...
| C3 | `PIP_LOCK.csv` lives at `inst/PIP_LOCK.csv`, read via `system.file()`; tests use `tempfile()` and a `path` arg | code review + tests | 3 |
...
- **Allowed**: `R/get_branches.R`, `R/install_pip_packages.R`, `R/init_metapip.R`,
  ...
  `tests/testthat/*`, regenerated `man/`+`NAMESPACE`.
